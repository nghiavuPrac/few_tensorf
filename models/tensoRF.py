...
class TensorVMSplit(TensorBase):
    ...
    def compute_densityfeature(self, xyz_sampled, mask=None):
        # plane + line basis
        coordinate_plane = torch.stack(
          (
            xyz_sampled[..., self.matMode[0]], 
        ...
        for idx_plane in range(len(self.density_plane)):
            ...
            if mask == None:
                ...
                den_mask = mask['den'][..., None]
                sigma_feature = sigma_feature + torch.sum(
                  (plane_coef_point*den_mask) * (line_coef_point*den_mask), dim=0
                )

        return sigma_feature


    def compute_appfeature(self, xyz_sampled, mask=None):
        # plane + line basis
        coordinate_plane = torch.stack(
          (
            xyz_sampled[..., self.matMode[0]], 
        ...
        for idx_plane in range(len(self.app_plane)):
            ...
            if mask == None:
                ...
                app_mask = mask['app'][..., None]
                plane_coef_point.append(plane_coef*app_mask)
                line_coef_point.append(line_coef*app_mask)
    ...
    def up_sampling_VM(self, plane_coef, line_coef, res_target):
        ...
        for i in range(len(self.vecMode)):
            ...
            plane_coef[i] = torch.nn.Parameter(
                F.interpolate(
                  plane_coef[i].data, size=(res_target[mat_id_1], res_target[mat_id_0]), mode='bilinear', align_corners=True
                )
            )
            line_coef[i] = torch.nn.Parameter(
                F.interpolate(
